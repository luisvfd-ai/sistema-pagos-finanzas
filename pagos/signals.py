...
from .models import PagoProgramado, EventoPago, PagoReal
from .calendar import generar_eventos_pago


# Guardamos snapshot para detectar cambios relevantes
...
def crear_o_regenerar_calendario(sender, instance, created, **kwargs):
    """
    - Si se crea: genera eventos (cuotas_restantes)
    - Si se edita: regenera SOLO si cambian campos que afectan el calendario
      (monto, fecha_inicio, frecuencia, cuotas_restantes)
    """
    if created:
...
# ============================================
# CONCILIACIÓN: PAGOS REALES -> EVENTOS
# ============================================

def conciliar_eventos_con_pagos_reales(pago: PagoProgramado):
    total_pagado = pago.pagos_realizados.aggregate(
        total=Sum('monto')
    )['total'] or Decimal('0.00')

    eventos = list(
        pago.eventos.order_by('fecha', 'id').only('id', 'monto', 'estado')
    )
...
def conciliar_eventos_al_guardar_pago_real(sender, instance, created, **kwargs):
    conciliar_eventos_con_pagos_reales(instance.pago)
