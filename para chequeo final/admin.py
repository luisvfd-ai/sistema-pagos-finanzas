...
class PagoAdmin(admin.ModelAdmin):
    list_display = (
        'nombre',
        'modo_programacion',
        'categoria_recurrente_label_admin',
        'tipo',
    ...
    list_filter = ('modo_programacion', 'categoria_recurrente_ref', 'tipo', 'unidad_negocio_ref', 'frecuencia', 'activo')
    search_fields = ('nombre', 'descripcion', 'unidad_negocio', 'categoria_recurrente')
    readonly_fields = ('anulado_en', 'anulado_por')

    fieldsets = (
        ('Base', {
            'fields': ('nombre', 'modo_programacion', 'tipo', 'categoria_recurrente_ref', 'categoria_recurrente', 'unidad_negocio_ref', 'unidad_negocio', 'descripcion', 'activo')
        }),
        ('Programación', {
            'fields': ('monto', 'fecha_inicio', 'frecuencia', 'total_cuotas', 'cuotas_restantes')
        }),
        ('Configuración recurrente', {
            'fields': ('indefinido', 'fecha_fin', 'dia_vencimiento', 'metodo_proyeccion', 'monto_proyeccion_manual')
        }),
        ('Anulación', {
            'fields': ('anulado_en', 'anulado_por', 'motivo_anulacion')
        }),
    )
    ...
    @admin.display(description='Categoría recurrente')
    def categoria_recurrente_label_admin(self, obj):
        return obj.categoria_recurrente_label_actual() if hasattr(obj, 'categoria_recurrente_label_actual') else obj.categoria_recurrente
